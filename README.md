...
打开 Locator Tab 后点击“拾取元素”，再到被调试页面点击输入框、按钮或其他 DOM 元素。插件会综合检查专用测试属性、稳定 `id`、`name`、ARIA、普通属性、class、文本、CSS 和 XPath。每个进入结果列表的候选都必须在当前 Document 或 Shadow Root 中唯一命中用户选中的元素；只匹配到相似元素或同时命中多个元素的定位不会被推荐。

如果基础 class、文本或属性命中多条，算法会继续寻找稳定祖先，或同一行、同一容器中的唯一链接和关联元素，将条件组合到唯一为止。面板按稳定性展示“最优、备选、兜底”三档，逐条给出定位类型、策略、分数、基础命中数量、上下文依据和扣分公式。推荐结果不会固定优先 CSS，也不会生成 `nth-child`、绝对数字 XPath、数字位置或屏幕坐标。

对于 Chrome 翻译自动插入的嵌套 `<font>`，插件会先还原到网站原本的元素；点击按钮或链接内部文字时，也会优先定位实际可操作控件。历史记录支持搜索、重复合并和当前页面重新验证，验证成功时会高亮唯一匹配元素。

可直接复制 Selenium Python、Java 或 JavaScript 代码。单层和嵌套 iframe 会保存由外到内的完整 frame 链并依次生成切换代码，Shadow DOM 结果会生成 Selenium 4 的逐层 Shadow Root 查找代码；普通全局 XPath 不会被错误地用于 Shadow DOM。DevTools 窄屏停靠时操作列保持可见。拾取监听只在用户主动开启期间存在，完成、按 Esc、切换 Tab 或隐藏面板后立即释放。产品取舍和验收标准见 [docs/locator-product-design.md](docs/locator-product-design.md)。
